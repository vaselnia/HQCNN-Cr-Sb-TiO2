...
# Suppress verbose Optuna logging and enforce deterministic seeding
...
# ------------------------------------------------------
# Quantum Layer Setup: Trainable Encoding + All-to-All QAOA Ansatz
# ------------------------------------------------------
n_qubits = X.shape[1]
n_layers = 4
dev = qml.device("default.qubit", wires=n_qubits, seed=SEED)  # Deterministic quantum backend

def qaoa_all_to_all(inputs, enc_scale, enc_shift, gamma, delta, beta):
    """
    QAOA-inspired parameterized quantum circuit:
    1. Trainable input angle encoding via scaled and shifted RY rotations.
    2. Multi-layer QAOA entangling block (all-to-all CNOT network).
    3. Parameterized single-qubit mixer/rotation gates (RX, RY, RZ).
    """
    for i in range(n_qubits):
        qml.RY(inputs[i] * enc_scale[i] + enc_shift[i], wires=i)
    for l in range(n_layers):
        # All-to-all entangling CNOT network
        for i in range(n_qubits):
