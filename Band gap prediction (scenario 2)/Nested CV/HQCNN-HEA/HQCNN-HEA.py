...
# Suppress verbose Optuna logging for cleaner fold output
...
# Drop non-feature identifiers and target-related metadata columns
X = dataset.drop(
...
# Scale target variable to [-1, 1] matching Pauli-Z expectation value bounds
scaler_y = MinMaxScaler(feature_range=(-1, 1))
y = scaler_y.fit_transform(y.reshape(-1, 1)).ravel()

# ======================================================
# Quantum Layer (Enhanced Hardware-Efficient Ansatz - HEA)
# ======================================================
n_qubits = X.shape[1]
n_layers = 4

# Initialize deterministic PennyLane simulator device
dev = qml.device("default.qubit", wires=n_qubits, seed=SEED)

def enhanced_hea(inputs, scale, shift, rot):
    # Trainable linear-scaled angle embedding
    for i in range(n_qubits):
        qml.RY(inputs[i] * scale[i] + shift[i], wires=i)

    # Entangling blocks with single-qubit Euler-like rotations and all-to-all CNOTs
    ...
    # Measure expectation values of Pauli-Z on all qubits
...
for fold, (train_idx, test_idx) in enumerate(outer_cv.split(X)):
    ...
    def objective(trial):
        ...
        for tr_idx, val_idx in inner_cv.split(X_train_outer):
            ...
            # Fresh PennyLane PyTorch layer instance per inner fold
            qlayer = qml.qnn.TorchLayer(
                qml.QNode(enhanced_hea, dev, interface="torch"),
                weight_shapes
            )

            # Hybrid Quantum-Classical architecture for inner validation
    ...
    # Run Optuna study using deterministic TPE Sampler and Median Pruner
    ...
    # 100-epoch full training loop on outer training split
    for _ in range(100):
        model.train()
        for xb, yb in loader:
            optimizer.zero_grad()
    ...
    # Invert scaling back to original physical target unit
...
print("==========================================")
```.append(mean_absolute_error(y_true, y_pred))
outer_r2.append(r2_score(y_true, y_pred))
