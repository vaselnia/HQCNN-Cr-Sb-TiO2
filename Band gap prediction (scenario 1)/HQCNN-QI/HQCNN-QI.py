...
def qaoa_all_to_all(inputs, enc_scale, enc_shift, gamma, delta, beta):
    # Trainable feature encoding via scaled and shifted RY rotations
    for i in range(n_qubits):
        qml.RY(inputs[i] * enc_scale[i] + enc_shift[i], wires=i)

    # Multi-layer QAOA-inspired ansatz with all-to-all entangling blocks
    for l in range(n_layers):
        # All-to-all CNOT entangling layer
        for i in range(n_qubits):
            for j in range(i+1, n_qubits):
                qml.CNOT(wires=[i, j])
        # Parameterized single-qubit rotation layer (RX, RY, RZ)
        for i in range(n_qubits):
...
# Export top-level and decomposed circuit diagrams
