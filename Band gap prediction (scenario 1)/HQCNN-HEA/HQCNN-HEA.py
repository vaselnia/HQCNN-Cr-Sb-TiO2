...
# ------------------------------
# Quantum Layer Setup (Hardware-Efficient Ansatz with Full Entanglement)
# ------------------------------
n_qubits = X.shape[1]
n_layers = 4
...
def enhanced_hea(inputs, scale, shift, rot):
    ...
    for l in range(n_layers):
        # Single-qubit parameterized rotations (RX, RY, RZ)
        for i in range(n_qubits):
            qml.RX(rot[l, i, 0], wires=i)
            qml.RY(rot[l, i, 1], wires=i)
            qml.RZ(rot[l, i, 2], wires=i)
        # Full entanglement block (all-to-all CNOT connectivity)
        for i in range(n_qubits):
