...
# ======================================================
# Fix random seeds and backend execution parameters to ensure 100% deterministic runs
SEED = 42
...
# Extract feature matrix X and target vector y (predicting 'y' property)
...
# Convert arrays to PyTorch TensorDatasets
...
# ------------------------------
# Classical neural module emulating quantum gate transformations using stacked Linear + Tanh layers
n_qubits = X.shape[1]
n_layers = 4
...
# ======================================================
# Optuna Hyperparameter Tuning (Deterministic)
# ======================================================
# Setup Optuna study with Tree-structured Parzen Estimator (TPE) and Median Pruner
...
def objective(trial):
    ...
    # Dynamic model definition for hyperparameter optimization trial
...
# Train Final Model
# ------------------------------
# Rebuild the network using the best hyperparameters found by Optuna
...
# Seeded generator for final DataLoader
...
for epoch in range(final_epochs):
    ...
    # Track and print test loss every 10 epochs
    model.eval()
    with torch.no_grad():
...
# ------------------------------
# Model inference on test set and inverse scaling to original units
...
# ------------------------------
# Plot True vs Predicted (Parity Plot)
# ------------------------------
...
# Equal aspect ratio for accurate diagonal alignment
...
plt.show()
fig.savefig('Classical_NN.png', dpi=300, bbox_inches='tight')
...
def plot_nn_professional_save(layer_sizes, filename="NN_architecture.png"):
    ...
    radius = 0.3
    font_size = 10
    vertical_text_offset = 2.5  # Vertical spacing for layer labels
    ...
    # Assign distinct colors and titles per layer type
    layer_colors = []
    layer_labels = []
...
# Define structure and generate the schematic diagram
