...
# ------------------------------------------------------
# Fix global random seeds to guarantee full deterministic execution across PyTorch and NumPy
SEED = 42
random.seed(SEED)
np.random.seed(SEED)
...
# Suppress Optuna info logs to keep console clean
...
# Extract feature matrix X and target vector y (excluding non-feature and metadata columns)
...
# ------------------------------------------------------
# Set number of qubits equal to the number of input features
n_qubits = X.shape[1]
n_layers = 4
# Initialize PennyLane default simulator device
...
def enhanced_hea(inputs, scale, shift, rot):
    ...
    for l in range(n_layers):
        # Single-qubit rotations (RX, RY, RZ) per wire
        for i in range(n_qubits):
            qml.RX(rot[l, i, 0], wires=i)
            qml.RY(rot[l, i, 1], wires=i)
            qml.RZ(rot[l, i, 2], wires=i)
        # Full Entanglement (all-to-all CNOT connectivity)
        for i in range(n_qubits):
...
# Shape definitions for the trainable quantum weights
weight_shapes = {
    "scale": n_qubits,
    "shift": n_qubits,
...
# Construct PyTorch-compatible quantum layer (TorchLayer)
qnode = qml.QNode(enhanced_hea, dev, interface="torch", diff_method="backprop")
qlayer = qml.qnn.TorchLayer(qnode, weight_shapes)

# ------------------------------------------------------
# Circuit Diagram Drawing and Saving
# ------------------------------------------------------
...
def objective(trial):
    ...
    # Dynamic trial model definition
...
# Run hyperparameter optimization using TPE Sampler & Median Pruner
...
# Final hybrid model using optimized hyperparameters
...
# ------------------------------------------------------
# Compute model predictions on test set and reverse scaling to original values
...
# Diagonal reference line y = x across the entire range
...
# Equal aspect ratio for accurate geometric comparison
