...
# Suppress verbose Optuna logging and set seeds
...
# ------------------------------------------------------
# Map inputs to [0, pi] for quantum rotational encoding
scaler_X = MinMaxScaler(feature_range=(0, np.pi))
X_scaled = scaler_X.fit_transform(X)

# Map targets to [-1, 1] aligned with Pauli-Z expectation range
...
# ------------------------------------------------------
# Quantum Layer: Trainable Encoding + All-to-All QAOA-like Ansatz
# ------------------------------------------------------
n_qubits = X.shape[1]
n_layers = 4
...
def qaoa_all_to_all(inputs, enc_scale, enc_shift, gamma, delta, beta):
    ...
    for l in range(n_layers):
        # All-to-all CNOT entanglement layer
        for i in range(n_qubits):
...
# Reference identity parity line (y = x) across the domain
