...
# Enforce system-level environment variables for strict determinism
os.environ["PYTHONHASHSEED"] = str(SEED)
os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"

# Seed all random number generators across backends
random.seed(SEED)
np.random.seed(SEED)
torch.manual_seed(SEED)
torch.cuda.manual_seed_all(SEED)

# Enforce deterministic PyTorch algorithms and cuDNN configurations
...
def objective(trial):
    ...
    # Deterministic DataLoader configuration
    g = torch.Generator()
    g.manual_seed(SEED)
...
# ------------------------------
# Inverse transform scaled predictions and ground truth to physical scale
...
def plot_nn_professional_save(layer_sizes, filename="NN_architecture.png"):
    ...
    radius = 0.3
    font_size = 10
    vertical_text_offset = 2.5  # Extended vertical text offset for labels
    ...
    for i, n_neurons in enumerate(layer_sizes):
        ...
        # Place layer text annotation with extended vertical offset
